...
export class Music {
    ...
    async play(command: CommandMessage, client: Client) {
        ...
        if (!permissions.has('CONNECT') || !permissions.has('SPEAK')) {
            embed.setTitle('J\'ai besoin de la permission de rejoindre et parler dans ton canal.')
            return command.reply(embed)
        }
    ...
    next(command: CommandMessage, client: Client) {
        ...
        if (!command.member.voice.channel) {
            embed.setTitle(`Tu dois être dans un canal vocal pour passer la musique !`)
            return command.reply(embed)
        }
        if (!serverQueue) {
    ...
    stop(command: CommandMessage, client: Client) {
        ...
        if (!command.member.voice.channel) {
            const embed = newMessage(command)
                .setTitle(`Tu dois être dans un canal vocal pour arrêter la musique !`)
            return command.reply(embed)
        }
        serverQueue.songs = []
        serverQueue.connection.dispatcher.end()
